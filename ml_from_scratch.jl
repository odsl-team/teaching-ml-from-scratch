...
pullback(δy, ::typeof(*), a, b) = @TODO

pullback(δy, ::typeof(sum), x) = δ∅, fill!(similar(x), δy)
pullback(δy, ::typeof(mean), x) = @TODO
...
function pullback(δz, fg::ComposedFunction, x)
    ...
    δf, δy = @TODO
    δg, δx = @TODO
    return ((outer = @TODO, inner = @TODO), @TODO)
end


function pullback(δy, f::Fix1, x2)
    δf_f, δf_x, δx2 = @TODO
    return ((f = @TODO, x = @TODO), @TODO)
end
...
pullback(δy, ::typeof(relu), x) = δ∅, @TODO
...
# ML Model structure:
# Fully connected layer, 18 inputs, 128 outputs
# ReLU activation
# Fully connected layer, 128 inputs, 128 outputs
# ReLU activation
# Fully connected layer, 128 inputs, 1 output
# Logistic activation
...
grad_model_loss = pullback(one(Float32), @TODO, @TODO)
